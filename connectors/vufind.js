            ...
            if (res && res.rss.channel[0].item) {
                ...
                request.get(res.rss.channel[0].item[0].guid[0]._, function (error, message, response) {
                    ...
                    $('.record .details h3').each(function (index, elem) {
                        responseHoldings.push({ library: $(this).text().trim(), available: $(this).next('table').find('span.available').length, unavailable: $(this).next('table').find('span.checkedout').length });
                    });
